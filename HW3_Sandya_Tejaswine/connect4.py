...
def minimax(player, board, depth_limit,max_player=True):
    """
    ...
    depth_limit: int
        the tree depth that the search algorithm needs to go further before stnext_playering
    max_player: boolean
...
###############################################################################
    def value(player, board, depth_limit,max_player):
        placement=None
        ...
        if depth_limit==0 or board.terminal() :
            ...
            if max_player==True:
                placement,score=max_value(player,board,depth_limit)
            else:
                placement,score=min_value(player,board,depth_limit)
    ...
    def max_value(player, board, depth_limit):
        next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        score=float("-inf")
        successor=get_child_boards(player,board)
        for i in successor:
            col,new_board=i
            newplacement,newscore=value(next_player, new_board, depth_limit-1,max_player=False)
            #newscore=evaluate(next_player,new_board)
            if newscore>score:
                score=newscore
                placement=col
        return placement,score
    ...
    def min_value(player, board, depth_limit):
        next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        score=float("+inf")
        succesoor=get_child_boards(player,board)
        for i in succesoor:
            col,new_board=i
            newplacement,newscore=value(next_player, new_board, depth_limit-1,max_player=True)
            #newscore=evaluate(next_player,new_board)
            if newscore<score:
                score=newscore
                placement=col
...
###############################################################################
    placement,score = value(player,board,depth_limit,max_player)
    return placement


def alphabeta(player, board, depth_limit,alpha=-math.inf,beta=math.inf,max_player=True):
    """
...
### Please finish the code below ##############################################
###############################################################################
    def value(player, board, depth_limit,alpha,beta,max_player):
        placement=None
        next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        if depth_limit==0 or board.terminal() :
            if max_player==True:
            ...
            if max_player==True:
                placement,score=max_value(player,board,depth_limit,alpha,beta)
            else:
                placement,score=min_value(player,board,depth_limit,alpha,beta)
        return placement,score 
    ...
    def max_value(player, board, depth_limit,alpha,beta):
        ...
        for i in successor:
            col,new_board=i
            newplacement,newscore=value(next_player, new_board, depth_limit-1,alpha,beta,max_player=False)
            #newscore=evaluate(next_player,new_board)
            if newscore>score:
                score=newscore
                placement=col
    ...
    def min_value(player, board, depth_limit,alpha,beta):
        ...
        for i in succesoor:
            col,new_board=i
            newplacement,newscore=value(next_player, new_board, depth_limit-1,alpha,beta,max_player=True)
            #newscore=evaluate(next_player,new_board)
            if newscore<score:
                score=newscore
                placement=col
...
###############################################################################
    placement,score = value(player,board,depth_limit,alpha,beta,max_player)
    return placement


def expectimax(player, board, depth_limit,max_player=True):
    """
    ...
    board: the current game board instance
    depth_limit: int
        the tree depth that the search algorithm needs to go before stnext_playering
    max_player: boolean
    ...
    def value(player, board, depth_limit,max_player):
        ...
        if depth_limit==0 or board.terminal() :
            ...
            if max_player==True:
                ...
                score=min_value(player,board,depth_limit)
        return placement,score 
